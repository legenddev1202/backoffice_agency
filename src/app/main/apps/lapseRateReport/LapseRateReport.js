...
import { selectProjects, getProjects , saveLapseRate, saveFireLapseRate, saveLifeLapseRate, saveHealthLapseRate } from './store/lapseSlice';
import { getAutoBonus, selectContacts } from './store/bonusPlanSlice';
import { getWidgets, selectWidgets } from './store/widgetsSlice';
import { rows, columns, levelColumns, firerows, liferows, healthrows, fireLevelColumns } from './tableDataTemplate';
...
function ProjectDashboardApp(props) {
			...
			if (Object.keys(projects[0]).includes('Auto')) {
				Object.keys(data.Auto).map(month => {
					Object.keys(data.Auto[month]).map(policy => {
						if (month !== 'id') {
							const {value} = data.Auto[month][policy];
			...
			if (Object.keys(projects[0]).includes('Fire')) {
				Object.keys(data.Fire).map(month => {
					Object.keys(data.Fire[month]).map(policy => {
						if (month !== 'id') {
							const {value} = data.Fire[month][policy];
			...
			if (Object.keys(projects[0]).includes('Life')) {
				Object.keys(data.Life).map(month => {
					Object.keys(data.Life[month]).map(policy => {
						if (month !== 'id') {
							const {value} = data.Life[month][policy];
			...
			if (Object.keys(projects[0]).includes('Health')) {
				Object.keys(data.Health).map(month => {
					Object.keys(data.Health[month]).map(policy => {
						console.log(policy);
						if (month !== 'id') {
							const {value} = data.Health[month][policy];
	...
	function getLevel(origin, value) {
		...
		} if (parseFloat(origin['level 5'].percent) === parseFloat(value)) {
			return 'level 5';
		}
		...
		if (title === 'Auto') {
			...
			if (month !== 'December') {
				...
				if (contacts.length > 0) {
					Object.keys(contacts[0]).map((item, index) => {
						if (item.includes('monthlyAgencyLapseAutoBonus')) {
							const alignedBonus = alignJson(contacts[0].monthlyAgencyLapseAutoBonus);
							const level = getLevel(alignedBonus, value);
			...
			if (month !== 'December') {
				let temp = {
					...
					[month]: {
						...
						[field]: { ...state.fireRows[month][field], value },
				...
				if (contacts.length > 0) {
					Object.keys(contacts[0]).map((item, index) => {
						if (item.includes('monthlyAgencyLapseFireBonus')) {
							...
							const alignedBonus = alignJson(contacts[0].monthlyAgencyLapseFireBonus);
							const level = getLevel(alignedBonus, value);
