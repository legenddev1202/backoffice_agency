...
import BarChart from 'app/main/components/widgets/BarChart';
...
function Component() {
	...
	const [title, setTitle] = useState('Producer Opportunity(Graph)');
	...
	useEffect(() => {
		widgets = {
			...
			data: [
				{
					proTable: widgets['Projections_Default_Producer_Opportunity_Graph_Production_Table'],
					incTable: widgets['Projections_Default_Producer_Opportunity_Graph_Income_Table']
				},
				{
					proTable: widgets['Projections_Default_Producer_Opportunity_Graph_Production_Table'],
					incTable: widgets['Projections_Default_Producer_Opportunity_Graph_Income_Table']
		...
		if (!_.isEmpty(widgets) && !_.isEmpty(main)) {			
			...
			if (widgets.Projections_Default_Producer_Opportunity_Graph_Production_Table) {
				...
				[0, 1, 2, 3, 4].map(number => {
					...
					widgets = {
						...widgets,
						Projections_Default_Producer_Opportunity_Graph_Production_Table: {
							...widgets.Projections_Default_Producer_Opportunity_Graph_Production_Table,
							table: {
								...widgets.Projections_Default_Producer_Opportunity_Graph_Production_Table.table,
								columns: [
									{
										...widgets.Projections_Default_Producer_Opportunity_Graph_Production_Table.table
											.columns[0],
					...
					widgets = {
						...widgets,
						Projections_Default_Producer_Opportunity_Graph_Income_Table: {
							...widgets.Projections_Default_Producer_Opportunity_Graph_Income_Table,
							table: {
								...widgets.Projections_Default_Producer_Opportunity_Graph_Income_Table.table,
			...
			if (widgets.Projections_Default_Producer_Opportunity_Graph_Chart && tableData.length > 0) {
				let datasets = [];

				['Salary', 'Auto', 'Fire', 'Life', 'Health', 'Bank', 'Lapse%'].map((policy, index) => {
					let chartData = [];
					tableData.map(data => {
						chartData.push(data.incTable.table.tableContent['Annual Totals'][policy]);
					});

					let dataset =
						widgets.Projections_Default_Producer_Opportunity_Graph_Chart.mainChart.TW.datasets[index];
					console.log('--===', dataset);
					datasets.push({
						...dataset,
						data: [...chartData]
				...
				widgets = {
					...widgets,
					Projections_Default_Producer_Opportunity_Graph_Chart: {
						...widgets.Projections_Default_Producer_Opportunity_Graph_Chart,
						mainChart: {
							...widgets.Projections_Default_Producer_Opportunity_Graph_Chart.mainChart,
							TW: {
								...widgets.Projections_Default_Producer_Opportunity_Graph_Chart.mainChart.TW,
								datasets: [...datasets]
							}
						}
					}
					...
					<div className="flex p-12">
						...
						>
							<div className="widget flex w-1/2 p-12">
								<BarChart data={data.widgets.Projections_Default_Producer_Opportunity_Graph_Chart} />
							</div>
						</FuseAnimateGroup>
					</div>
				</div>
