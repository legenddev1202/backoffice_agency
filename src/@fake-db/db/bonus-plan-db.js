...
mock.onGet('/api/bonus-plan/contacts').reply(config => {
...
mock.onGet('/api/bonus-plan/user').reply(config => {
	return [200, contactsDB.user[0]];
});

mock.onPost('/api/bonus-plan/add-contact').reply(request => {
...
mock.onPost('/api/bonus-plan/update-contact').reply(request => {
	const { contact } = JSON.parse(request.data);
...
mock.onPost('/api/bonus-plan/remove-contacts').reply(request => {
...
mock.onPost('/api/bonus-plan/toggle-starred-contact').reply(request => {
...
mock.onPost('/api/bonus-plan/toggle-starred-contacts').reply(request => {
	const { contactIds } = JSON.parse(request.data);
	contactsDB.user[0].starred = _.xor(contactsDB.user[0].starred, contactIds);
	return [200, contactIds];
});

mock.onPost('/api/bonus-plan/set-contacts-starred').reply(request => {
	const { contactIds } = JSON.parse(request.data);
...
mock.onPost('/api/bonus-plan/set-contacts-unstarred').reply(request => {
	const { contactIds } = JSON.parse(request.data);
